...
public class Vehicle10BodyScript : MonoBehaviour {
	...
	void Awake()
	{
		bs = PlayerPrefs.GetInt ("V10B");
		ss = PlayerPrefs.GetInt ("V10S");
		es = PlayerPrefs.GetInt ("V10E");
		cs = PlayerPrefs.GetInt ("V10C");
	}

	void Start()
	{

		onNew = PlayerPrefs.GetInt ("V10N");
		if (onNew == 0) {
			cs = 1;
			PlayerPrefs.SetInt ("V10N", 1);
		}
	...
	void FixedUpdate()
	{
		...
		PlayerPrefs.SetInt ("V10B", bs);
		PlayerPrefs.SetInt ("V10E", es);
		PlayerPrefs.SetInt ("V10S", ss);
		PlayerPrefs.SetInt ("V10C", cs);
