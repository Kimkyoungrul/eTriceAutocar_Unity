...
public class P2wheel : MonoBehaviour {
...
//	private bool shrinkFireTrail = false;
	...
	void Update () 
	{
		...
		if (frontTire) {
			//steer tire visual//
			turnObj.transform.localRotation = Quaternion.Euler (wheelObj.transform.localRotation.x, vehicleObj.GetComponent<P2vehicleController> ().inputX * 33f, wheelObj.transform.localRotation.z);
		}
	...
	void setTouchGround(bool _bool)
	{
		...
		if(wheelLB)
		{
			vehicleObj.GetComponent<P2vehicleController>().touchLB = touchGround;
		}
		else if(wheelLF)
		{
			vehicleObj.GetComponent<P2vehicleController>().touchLF = touchGround;
		}
		else if(wheelRB)
		{
			vehicleObj.GetComponent<P2vehicleController>().touchRB = touchGround;
		}
		else if(wheelRF)
		{
			vehicleObj.GetComponent<P2vehicleController>().touchRF = touchGround;
		}
	}
}
