...
public class wheelPlayer1 : MonoBehaviour {
...
//	private bool shrinkFireTrail = false;
	...
	void Update () 
	{
		...
		if (frontTire) {
			//steer tire visual//
			turnObj.transform.localRotation = Quaternion.Euler (wheelObj.transform.localRotation.x, vehicleObj.GetComponent<P1vehicleController> ().inputX * 33f, wheelObj.transform.localRotation.z);
		}
	...
	void setTouchGround(bool _bool)
	{
		...
		if(wheelLB)
		{
			vehicleObj.GetComponent<P1vehicleController>().touchLB = touchGround;
		}
		else if(wheelLF)
		{
			vehicleObj.GetComponent<P1vehicleController>().touchLF = touchGround;
		}
		else if(wheelRB)
		{
			vehicleObj.GetComponent<P1vehicleController>().touchRB = touchGround;
		}
		else if(wheelRF)
		{
			vehicleObj.GetComponent<P1vehicleController>().touchRF = touchGround;
		}
	}
}
