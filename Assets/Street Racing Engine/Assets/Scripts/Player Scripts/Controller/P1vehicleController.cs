...
public class P1vehicleController : MonoBehaviour {
	...
	void Awake()
	{
		...
		Instantiate(Resources.Load ("Manager/MultiplayerCanvas", typeof(GameObject)));
		Instantiate(Resources.Load ("Manager/P1CameraDown", typeof(GameObject)));

		move = true;

		if (gameObject.name == "P1Vehicle07") {
			suspension = 0;
		}

		vehicleBody = GameObject.Find ("P1Body");
		wheelLeftFront = GameObject.Find ("P1FLW");
		wheelRightFront = GameObject.Find ("P1FRW");
		wheelLeftBack = GameObject.Find ("P1BLW");
		wheelRightBack = GameObject.Find ("P1BRW");
		tirePhysicsMat = GameObject.Find ("P1TireP").GetComponent<BoxCollider> ().material;
		bodyPhysicsMat = GameObject.Find ("P1BodyP").GetComponent<BoxCollider> ().material;

		//make sure vehicleBody has the correct parent//
		vehicleBody.transform.SetParent(transform);
		
		//create physics body//
		physicsBody = new GameObject(gameObject.name+" physicsP1");
		...
		wheels = new GameObject(gameObject.name+" wheelsP1");
		...
		wheelPlayer1 wheelScript = colLB.AddComponent<wheelPlayer1>();
		...
		wheelScript = colLF.AddComponent<wheelPlayer1>();
		wheelScript.vehicleObj = gameObject;
		wheelScript.wheelObj = wheelLeftFront;
		wheelScript.wheelLF = true;
		wheelScript.wheelHangDistance = wheelHangDistance;
		
		turnLF = new GameObject("P1turnLF");
		...
		wheelScript = colRB.AddComponent<wheelPlayer1>();
		wheelScript.vehicleObj = gameObject;
		...
		wheelScript = colRF.AddComponent<wheelPlayer1>();
		wheelScript.vehicleObj = gameObject;
		...
		suspensionBody = new GameObject(gameObject.name+" suspensionP1");
		...
		colSuspension = new GameObject("P1colSuspension");
		...
		Nitro = GameObject.Find ("P1NitroSound").GetComponent<AudioSource> ();
		FireIgnition = GameObject.Find ("P1FireIgnitionSound").GetComponent<AudioSource> ();
		Tire = GameObject.Find ("P1TireSound").GetComponent<AudioSource> (); 
	...
	void Start () 
	{
		...
		Physics.defaultContactOffset = 100f;
		...
		NitroImage = GameObject.Find ("P1NitroImage").GetComponent<Slider> ();
		NitroParticles = GameObject.Find ("P1Nitro");
		NitroParticles.SetActive (false);
		FireIgnitionParticles = GameObject.Find ("P1FireIgnition");
	...
	void FixedUpdate () 
	{
		...
		//input//
		if (move) {
			inputX = Mathf.SmoothStep (inputX, CrossPlatformInputManager.GetAxis ("P1Horizontal"), (steering * 0.2f) * Time.deltaTime);
			inputY = CrossPlatformInputManager.GetAxis ("P1Vertical");
		}else {
			inputX = 0f;
			inputY = 0f;
		}
		...
		if (inputY > 0) {
			...
			if (Input.GetKey (KeyCode.LeftShift) && NitroImage.value > 2) {
